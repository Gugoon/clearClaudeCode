...
```mermaid
flowchart TD
    A["<b>1. 단계 선택</b><br/>옵션(-1/-2/-3) 또는 대화형 메뉴로 Level 결정"]
    B["<b>2. 탐색</b><br/>• Level별로 정의된 경로 검사 (파일/디렉토리/심볼릭)<br/>• 글롭 패턴(plist, Group Containers 등) 매칭<br/>• Level 3: npm/yarn/pnpm/bun/brew 글로벌 패키지 조회<br/>• Level 3: command -v -a 로 PATH 보강 검색"]
    C["<b>3. 표시</b><br/>발견된 항목과 각 크기 출력"]
    D["<b>4. 확인</b><br/>• -y 없으면 'yes' 입력 요구<br/>• -n 이면 여기서 종료"]
    E["<b>5. 실행</b><br/>• 실행 중인 claude 프로세스 종료<br/>• (Level 3) 패키지 매니저로 글로벌 패키지 제거<br/>• (Level 3) Homebrew formula/cask 제거<br/>• rm -rf로 모든 경로 삭제 (필요 시 sudo 폴백)"]
